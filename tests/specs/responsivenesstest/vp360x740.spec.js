...
test.describe('Responsiveness test for ViewPort Size:( 360 x 740 )',async()=>{
    test.use({ viewport: { width: 360, height: 740 } });
    test.beforeAll(async () => {
      token = await common.getLoginToken(loginData.ValidLoginData.Email, loginData.ValidLoginData.Password)
    })
    test('Verify for ViewPort Size:( 360 x 740 ) the mail will be hide from the page', async ({ loginPage }) => {
    ...
    test('Verify for ViewPort Size:( 360 x 740 ) a menu icon will be visible instead of four options,  four options will be shown inside the menu icon', async ({ loginPage,dashboardPage }) => {
