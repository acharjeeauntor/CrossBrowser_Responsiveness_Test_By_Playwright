...
test.describe('Responsiveness test for ViewPort Size:( 1024 x 1366 )',async()=>{
    test.use({ viewport: { width: 1024, height: 1366 } });
    test.beforeAll(async () => {
      token = await common.getLoginToken(loginData.ValidLoginData.Email, loginData.ValidLoginData.Password)
    })
    test('Verify for ViewPort Size:( 1024 x 1366 ) the mail will be hide from the page', async ({ loginPage }) => {
  ...
  test('Verify for ViewPort Size:( 1024 x 1366 ) four tab will be visible properly', async ({ loginPage,dashboardPage }) => {
