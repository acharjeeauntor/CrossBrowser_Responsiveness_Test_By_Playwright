...
test.describe('Responsiveness test for ViewPort Size:( 412 x 732 )',async()=>{
    test.use({ viewport: { width: 412, height: 732 } });
    test.beforeAll(async () => {
      token = await common.getLoginToken(loginData.ValidLoginData.Email, loginData.ValidLoginData.Password)
    })
    test('Verify for ViewPort Size:( 412 x 732 ) the mail will be hide from the page', async ({ loginPage }) => {
    ...
    test('Verify for ViewPort Size:( 412 x 732 ) a menu icon will be visible instead of four options,  four options will be shown inside the menu icon', async ({ loginPage,dashboardPage }) => {
